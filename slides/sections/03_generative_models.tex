...
\begin{frame}[plain]
    ...
    \begin{tikzpicture}
        \node[draw=mainblue!90, fill=mainblue!10, rounded corners=8pt, inner sep=16pt, minimum width=0.82\paperwidth, align=center, thick] {
            {\fontsize{9pt}{11pt}\selectfont \bfseries \color{accentblue} SECTION 3}\\[6pt]
            {\fontsize{18pt}{22pt}\selectfont \bfseries \color{mainblue} Generative Model Families}\\[8pt]
            {\fontsize{8.5pt}{10.5pt}\selectfont \color{black!70} Variational Autoencoders (VAEs), Pix2Pix GANs \& Score-based Diffusion Models}
        };
    \end{tikzpicture}
    \vfill
\end{frame}
...
\begin{equation*}
\begin{aligned}
\log p_\theta(x) \ge \; & \E_{q_\phi(z|x)}[\log p_\theta(x|z)] \\
& - \KL{q_\phi(z|x)}{p(z)}
\end{aligned}
\end{equation*}
{\fontsize{7pt}{8pt}\selectfont
\begin{itemize}\setlength{\itemsep}{0pt}\setlength{\parskip}{0pt}
    \item \textbf{Likelihood}: Reconstruction fidelity (MSE/BCE).
    \item \textbf{KL Penalty}: Prior regularization ($p(z) = \mathcal{N}(0, I)$).
\end{itemize}
}
...
\begin{lstlisting}[basicstyle=\fontsize{6.5pt}{7.5pt}\ttfamily]
