...
\begin{frame}[plain]
    ...
    \begin{tikzpicture}
        \node[draw=mainblue!90, fill=mainblue!10, rounded corners=8pt, inner sep=16pt, minimum width=0.82\paperwidth, align=center, thick] {
            {\fontsize{9pt}{11pt}\selectfont \bfseries \color{accentblue} SECTION 4}\\[6pt]
            {\fontsize{18pt}{22pt}\selectfont \bfseries \color{mainblue} Virtual Staining Evaluation Metrics}\\[8pt]
            {\fontsize{8.5pt}{10.5pt}\selectfont \color{black!70} VIRVS Benchmark: PSNR, SSIM, PCC, MAE \& Cell-level Reporter Quantification}
        };
    \end{tikzpicture}
    \vfill
\end{frame}
